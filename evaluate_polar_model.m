function [ strct ] = evaluate_polar_model( varargin )
...
ploterror(z_ctl-z_reg(randperm(numel(z_reg))),F1,F2,4);
ploterror(z-z_reg,F1,F2,1);

% z_loc=z_reg;

% ni model
e_reg=net_struct.model_ni.net(t')';
sig_reg=net_struct.model_niloc.sig;
z_loc=net_struct.model_niloc.net(loc')';
z_reg=z-e_reg;
...
% p_ni_ctl=l_ni_ctl.*prior./normz_ctl;
p_ni_ctl=prior;
...
for i=1:acclag
    p_acc=pshift.*movprod(lratio,i)./p_ni;
    pshift=[nan;pshift(1:end-1)];
    acc_ratio(i,:)=quantile(p_acc,pq);
    pconv(i)=sum(p_acc>1)/numel(p_acc);
end
...
AL=-log(pval_ni);
% AL(pval_ni==0)=2*nanmax(-log(pval_ni(pval_ni>0)));
figure, plot2d;
set(gca,'Alim',[-log(0.05) nanmax(AL(:))]);
ploterror(z_reg-z_loc,F1,F2,2);
...
%FB model
e_reg=net_struct.model_fbni.net([t fb]')';
sig_reg=net_struct.model_fbloc.sig;
z_loc=net_struct.model_fbloc.net(loc')';
z_reg=z-e_reg;
z_reg_ctl=z_ctl-e_reg;
l_fb=(1/sqrt(2*pi*sig_reg^2))*exp(-(z_reg-z_loc).^2/(2*sig_reg^2));
l_fb_ctl=(1/sqrt(2*pi*sig_reg^2))*exp(-(z_reg_ctl-z_loc).^2/(2*sig_reg^2));
p_fb=l_fb.*prior./normz;
% p_fb_ctl=l_fb_ctl.*prior./normz_ctl;
p_fb_ctl=prior;
[pval_fb,N0,pmean_fb,pmean_fb_ctl]=pval2d(p_fb,p_fb_ctl);

lratio=l_fb./zcond;
pshift=[nan;p_fb(1:end-1)];
for i=1:acclag
    p_acc=pshift.*movprod(lratio,i)./p_fb;
    pshift=[nan;pshift(1:end-1)];
    acc_ratio(i,:)=quantile(p_acc,pq);
    pconv(i)=sum(p_acc>1)/numel(p_acc);
end
...
Mz=log(pmean_fb);
pval_fb(pval_fb<0.01)=0.01;
AL=-log(pval_fb);
% AL(pval_ni==0)=2*nanmax(-log(pval_ni(pval_ni>0)));
figure, plot2d;
set(gca,'Alim',[-log(0.05) nanmax(AL(:))]);
ploterror(z_reg-z_loc,F1,F2,5);
...
%                     pval(i,j)=boottest(p(idx),p_ctl(idx),N);
    ...
    function plot2d()
        if(isfield(params,'image'))
            ...
            ix=([1:sX]-sX/2)*pxl2mm;
            iy=(-[1:sY]+sY/3)*pxl2mm;
            imagesc(ix,iy,255*(1-fcolor(1))-params.image);
        end
...
%         colorbar;
    ...
    function ploterror(e,F1,F2,k)
        ...
        figure(F2);
        H=plot(lags,xc*evar);
        set(H,'Color',COL(k,:),'LineWidth',3,'MarkerSize',params.msize);        
        hold on;
        set(gca,'FontSize',params.fontsize)        
        set(gcf,'Color',fcolor);
        set(gca,'Color',fcolor,'XColor',1-fcolor,'YColor',1-fcolor);
