function [ F ] = plot_tuning2d_correlations( varargin )
...
hold on;
S=surf(params.x_bins*pxl2mm,params.y_bins*pxl2mm,ones(size(c')),...
    'CData',c',...
...
function [pval,c,M,B]=tuning2d()
% Mx=numel(params.x_edges)-1;
% My=numel(params.y_edges)-1;
c=zeros(params.x_nbins,params.y_nbins);
pval=nan(params.x_nbins,params.y_nbins);
M=nan(params.x_nbins,params.y_nbins);
B=nan(params.x_nbins,params.y_nbins);
...
for i=1:params.x_nbins
    for j=1:params.y_nbins
        ...
        if(numel(idx)>0 & params.t_col) %there is a target variable
            Ind=idx(~isnan(t(idx)) & ~isnan(z(idx)));
            [c(i,j),pval(i,j)]=corr(t(Ind),z(Ind));
            [r,M(i,j),B(i,j)]=regression(t(Ind)-nanmean(t),z(Ind),'one');
        end
    end
end
end
